...
//根据用户的设置构建背景换肤菜单
var menus = [];
for(var i = 0; i < desktopbgs.length; i++) {
	var deskbg = desktopbgs[i];
	var menu = {
        text: deskbg.title,
        func: function() {
            $(this).attr("style",'background:url('+deskbg.src+') repeat right bottom transparent;');
        }
	};
	menus.push(menu);
}

deskbgsmenu.push(menus);
...
var GetTaskSystem = function(obj){
    if(!_cache.TaskSystem){
    	...
    	for(var i = 0; i < taskmenus.length; i++) {
    		var dialog = taskmenus[i].dialog;
    		if(dialog) {
    			html += '<li><a menu="close" href="javascript:' + taskmenus[i].fn + ';" data-toggle="modal" data-target="#' + dialog + '">' + taskmenus[i].title + '</a></li>';
    		}else {
    			html += '<li><a menu="close" href="javascript:' + taskmenus[i].fn + ';">' + taskmenus[i].title + '</a></li>';
    		}
    	}
    	
        _cache.TaskSystem = $('<div class="popup-menu task-menu" style="z-index:99999;bottom:30px;display:none"><ul>'+html+'</ul></div>');
